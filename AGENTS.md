...
Build Your Own PagerDuty App. AI Transcript Logging
...
This is the application repository for the **HackerRank Build Your Own PagerDuty** assignment. The app is not a timed challenge: there is no start time and no end time.

Build and submit a PagerDuty-inspired incident management app that helps on-call teams receive events, manage incidents, and coordinate response. The app is full-stack React + Django (Python), with MongoDB for persistence. All implementation work described by this file belongs to this app.
...
| Platform | Path |
|---|---|
| macOS / Linux | `$HOME/hackerrank_pagerduty/log.txt` |
| Windows | `%USERPROFILE%\hackerrank_pagerduty\log.txt` |
...
Welcome to HackerRank Build Your Own PagerDuty. You are going to design, build, and ship a PagerDuty-inspired incident management app that helps on-call teams receive events, manage incidents, and coordinate response. Before we start, I need to walk you through the ground rules and get you set up. This takes about a minute.
...
├── backend/                          # Django API, kept from the Django sample calendar repo
