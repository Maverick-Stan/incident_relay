...
The PagerDuty app is to be built referring to a sample calendar repo. Your repo must keep the same structure, setup flow, formatting, and coding conventions as that sample. The validator in `skills/validate/SKILL.md` checks all of it against `GUIDELINES.md`.

## 1. Clone and study the Django sample

Clone the React + Django (Python) sample calendar repo as a reference before you begin. Use it to understand the app's structure, setup flow, formatting, coding conventions, and UI quality, then build the PagerDuty app in this repository. The sample is a reference only; it is not the app destination.

```bash
git clone https://github.com/ProblemSetters/coderepo-react-django-calendar.git ../coderepo-react-django-calendar-reference
```
...
Run the cloned sample and inspect its frontend, backend, manifests, scripts, configuration, and README so you understand the standards you must preserve while building the PagerDuty app.
```bash
